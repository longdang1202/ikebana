...
vc_map( array(
	...
	'params' => array(
		...
		array(
			...
			'admin_label' => true,
			'description' => sprintf( wp_kses(__( 'Enter link to video (Note: read more about available formats at WordPress <a href="%s" target="_blank">codex page</a>).', 'js_composer' ), array('a' => array('href' => array(), 'target' => array()))), 'http://codex.wordpress.org/Embeds#Okay.2C_So_What_Sites_Can_I_Embed_From.3F' )
		),
		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Extra class name', 'shopme' ),
		...
		'params' => array(
			...
			array(
				...
				'dependency' => array(
					...
					'value' => array('filter_cat')
				),
				'description' => __('The Page will then show products from only those categories.', 'shopme')
			),
			array(
				'type' => 'checkbox',
