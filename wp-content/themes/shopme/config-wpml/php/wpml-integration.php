...
// defaults: skip_missing = 1, before =  esc_html__('This post is also available in: '), after = ''
function wpml_content_languages($args=''){
    parse_str($args);
    if(function_exists('icl_get_languages')){
        $languages = icl_get_languages($args);
        if(1 < count($languages)){
            echo isset($before) ? $before : esc_html__('This post is also available in: ', 'sitepress');
            foreach($languages as $l){
                if(!$l['active']) $langs[] = '<a href="'. esc_url($l['url']) .'">'.$l['translated_name'].'</a>';
            }
            echo join(', ', $langs);
    ...
    if(!function_exists('icl_link_to_element')){    
        switch($element_type){
            ...
            case 'post_tag':
                ...
                $ret = '<a href="'. esc_url(get_tag_link($element_id)) .'">' . $tag->name . '</a>';
            case 'category':
                $ret = '<a href="'. esc_url(get_tag_link($element_id)) .'">' . get_the_category_by_ID($element_id) . '</a>';
            default: $ret = '';           
        }
        if($echoit){
            echo $ret;
...
function wpml_default_link($anchor){
    ...
    if($qv){
        $link = '<a href="' . esc_url($sitepress->language_url($sitepress->get_default_language()) . $url_glue . $qv) . '" rel="nofollow">' . esc_html($anchor) . '</a>';
    }else{
        $link = '';
    } 
