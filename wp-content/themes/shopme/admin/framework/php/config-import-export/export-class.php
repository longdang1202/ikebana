...
if (!class_exists('SHOPME_WP_THEME_SETTINGS_EXPORT')) {

	class SHOPME_WP_THEME_SETTINGS_EXPORT  {
		...
		public function init() {
			...
			echo '<pre>'."\n"; echo '$options = "'; print_r($export); echo '";</pre>';
       	...
       	public function generateExportFile($export_data) {
			...
			$export_file = sanitize_file_name(SHOPME_PREFIX .'theme-settings-'. $name_file .'.txt');
		...
		public function metaData() {
			...
			$meta_settings = $wpdb->get_results("SELECT * FROM {$table_name}", ARRAY_A);
