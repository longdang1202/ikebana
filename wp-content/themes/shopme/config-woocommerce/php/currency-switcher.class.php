	...
	class SHOPME_WC_CURRENCY_SWITCHER extends SHOPME_WOOCOMMERCE_CONFIG {
		...
		function __construct() {
			...
			$this->settings = array(
				...
				array( 'name' => esc_html__( 'Open Exchange Rate API', 'woothemes' ), 'type' => 'title', 'desc' => '', 'id' => 'product_enquiry' ),
				array(
					'name' => esc_html__('App Key', 'wc_enquiry_form'),
					'desc' 		=> sprintf( wp_kses(__('(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'wc_enquiry_form'), array('a' => array('href' => array()))), 'https://openexchangerates.org/signup' ),
		...
		public function enqueue_currency_js() {
			...
			wp_localize_script( SHOPME_PREFIX . 'woocommerce-mod', 'wc_currency_converter_params', array(
				...
				'i18n_oprice'      => esc_html__( 'Original price:', 'wc_currency_converter'),
