...
function latest_tweets_render( $screen_name, $count, $rts, $ats, $pop = 0 ){
    try {
        ...
        if( ! twitter_api_configured() ){
            throw new Exception( esc_html__('Plugin not fully configured','twitter-api') );
        }
...
class Latest_Tweets_Widget extends WP_Widget {
    ...
    public function __construct( $id_base = false, $name = '', $widget_options = array(), $control_options = array() ){
        ...
        $this->options = array(
            array (
                'name'  => 'title',
                'label' => esc_html__('Widget title', 'twitter-api'),
                'type'  => 'text'
            ),
            array (
                'name'  => 'screen_name',
                'label' => esc_html__('Twitter handle','twitter-api'),
                'type'  => 'text'
            ),
            array (
                'name'  => 'num',
                'label' => esc_html__('Number of tweets','twitter-api'),
                'type'  => 'number'
            ),
            array (
                'name'  => 'pop',
                'label' => esc_html__('Minimum popularity','twitter-api'),
                'type'  => 'number'
            ),
            array (
                'name'  => 'rts',
                'label' => esc_html__('Show Retweets','twitter-api'),
                'type'  => 'bool'
            ),
            array (
                'name'  => 'ats',
                'label' => esc_html__('Show Replies','twitter-api'),
                'type'  => 'bool'
            ),
        );
        $name or $name = esc_html__('Latest Tweets','twitter-api');
    ...
    private function check_instance( $instance ){
        ...
        $instance += array (
            'title' => esc_html__('Latest Tweets','twitter-api'),
            'screen_name' => '',
            'num' => 5,
            'pop' => 0,
            'rts' => '',
...
if( is_admin() ){
    ...
    function latest_tweets_plugin_row_meta( $links, $file ){
        if( false !== strpos($file,'/latest-tweets.php') ){
            $links[] = '<a href="options-general.php?page=twitter-api-admin"><strong>'.esc_html__('Connect to Twitter','twitter-api').'</strong></a>';
        } 
        return $links;
    }
