...
<div id="reviews" class="section_offset">
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->id ) ) : ?>
				...
				$comment_form = array(
					'title_reply'          => have_comments() ? esc_html__( 'Write Your Own Review', 'woocommerce' ) : __( 'Be the first to review', 'woocommerce' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
					'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'woocommerce' ),
					'comment_notes_before' => '',
					'comment_notes_after'  => '',
					'fields'               => array(
						'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__( 'Nickname', 'woocommerce' ) . ' <span class="required"></span></label> ' .
				...
				if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
					...
					$comment_form['comment_field'] .= '<p class="comment-form-desc">'. sprintf( esc_html__('You\'re reviewing: %s . How do you rate this product? *', 'woocommerce'), get_the_title() ) .'</p>';
