	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'installing'                      => esc_html(__( 'Installing Plugin: %s', 'ri-quartz' )),
				...
				'dashboard'                       => esc_html(__( 'Return to the dashboard', 'ri-quartz' )),
				...
				'plugin_already_active'           => esc_html(__( 'No action taken. Plugin %1$s was already active.', 'ri-quartz' )),
				'plugin_needs_higher_version'     => esc_html(__( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'ri-quartz' )),
				'complete'                        => esc_html(__( 'All plugins installed and activated successfully. %1$s', 'ri-quartz' )),
				'dismiss'                         => esc_html(__( 'Dismiss this notice', 'ri-quartz' )),
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'ri-quartz' ),
				esc_html__( 'Update Required', 'ri-quartz' )
			);
			return $actions;
		}
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'ri-quartz' ) );
			}
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'ri-quartz' ) . '</a>' ), '</p>';
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'ri-quartz' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'ri-quartz' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );

					}
				} elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'ri-quartz' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'ri-quartz' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_group ) {
					...
					$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'ri-quartz' ) . ' ' . $last_plugin );

					$rendered .= sprintf(
						$line_template,
						sprintf(
							translate_nooped_plural( $this->strings[ $type ], $count, 'ri-quartz' ),
							$imploded,
							$count
						)
					);
				...
				if ( current_user_can( 'install_plugins' ) ) {
					if ( $install_link_count > 0 ) {
						$action_links['install'] = sprintf(
							$link_template,
							translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'ri-quartz' ),
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			...
			if ( true === $required ) {
				return esc_html(__( 'Required', 'tgmpa' ));
			}

			return esc_html(__( 'Recommended', 'tgmpa' ));
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = esc_html(__( 'WordPress Repository', 'tgmpa' ));
					break;

				case 'external':
					$string = esc_html(__( 'External Source', 'tgmpa' ));
					break;

				case 'bundled':
					$string = esc_html(__( 'Pre-Packaged', 'tgmpa' ));
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return esc_html(__( 'Not Installed', 'tgmpa' ));
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = esc_html(__( 'Installed But Not Activated', 'tgmpa' ));
			} else {
				$install_status = esc_html(__( 'Active', 'tgmpa' ));
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html(__( 'Required Update not Available', 'tgmpa' ));

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = esc_html(__( 'Requires Update', 'tgmpa' ));

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html(__( 'Update recommended', 'tgmpa' ));
			}
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html(__( 'Installed version:', 'tgmpa' )) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . esc_html(__( 'Minimum required version:', 'tgmpa' )) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html(__( 'Available version:', 'tgmpa' )) . '</p>',
					$color,
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => esc_html(__( 'Plugin', 'tgmpa' )),
				'source' => esc_html(__( 'Source', 'tgmpa' )),
				'type'   => esc_html(__( 'Type', 'tgmpa' )),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = esc_html(__( 'Version', 'tgmpa' ));
			}

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['status'] = esc_html(__( 'Status', 'tgmpa' ));
			}
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = esc_html(__( 'Install', 'tgmpa' ));
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = esc_html(__( 'Update', 'tgmpa' ));
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = esc_html(__( 'Activate', 'tgmpa' ));
				}
			}
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html(__( 'No plugins are available to be installed at this time.', 'tgmpa' ));
					} else {
						$message = esc_html(__( 'No plugins are available to be updated at this time.', 'tgmpa' ));
					}
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						...
						$this->strings['activation_failed']  = esc_html(__( 'Plugin activation failed.', 'tgmpa' ));
						$this->strings['activation_success'] = esc_html(__( 'Plugin activated successfully.', 'tgmpa' ));
					}
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							parent::add_strings();
							$this->upgrader->strings['skin_before_update_header'] = esc_html(__( 'Updating Plugin %1$s (%2$d/%3$d)', 'tgmpa' ));
							...
							$this->upgrader->strings['skin_update_failed']       = esc_html(__( 'The installation of %1$s failed.', 'tgmpa' ));

							if ( $this->tgmpa->is_automatic ) {
								// Automatic activation strings.
								$this->upgrader->strings['skin_upgrade_start']        = esc_html(__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'tgmpa' ));
								$this->upgrader->strings['skin_update_successful']    = esc_html(__( '%1$s installed and activated successfully.', 'tgmpa' )) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']          = esc_html(__( 'All installations and activations have been completed.', 'tgmpa' ));
								$this->upgrader->strings['skin_before_update_header'] = esc_html(__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'tgmpa' ));

							} else {
								// Default installation strings.
								$this->upgrader->strings['skin_upgrade_start']        = esc_html(__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'tgmpa' ));
								$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'tgmpa' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']          = esc_html(__( 'All installations have been completed.', 'tgmpa' ));
								$this->upgrader->strings['skin_before_update_header'] = esc_html(__( 'Installing Plugin %1$s (%2$d/%3$d)', 'tgmpa' ));
							}
						}
					}
