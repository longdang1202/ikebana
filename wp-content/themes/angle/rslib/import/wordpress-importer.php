...
<?php
	...
	function backfill_parents() {
		...
		foreach ( $this->post_orphans as $child_id => $parent_id ) {
			...
			if ( $local_child_id && $local_parent_id )
				$wpdb->update( $wpdb->posts, array( 'post_parent' => esc_sql($local_parent_id) ), array( 'ID' => esc_sql($local_child_id) ), '%d', '%d' );
		}
	...
	function backfill_attachment_urls() {
		...
		foreach ( $this->url_remap as $from_url => $to_url ) {
			...
			$wpdb->query( $wpdb->prepare("UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s)", esc_sql($from_url), esc_sql($to_url)) );
			// remap enclosure urls
			$result = $wpdb->query( $wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure'", esc_sql($from_url), esc_sql($to_url)) );
		}
	}
...
function rs_wordpress_importer_init() {
...
add_action( 'admin_init', 'rs_wordpress_importer_init' );
